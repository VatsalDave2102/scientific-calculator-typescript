import { calculateBtn, CLOSEPARANTHESE, dataFormula, dataValue, E, emptyInput, FACTORIAL, isDegree, MATHCBRT, MATHLOG, MATHLOG10, MATHSQRT, numbers, OPENPARANTHESE, OPERATORS, output, PI, POWER, search, } from "./utils.js";
...
function correctPowerPosition(input) {
...
function correctFactPosition(input) {
...
function getPowerBase(formulas, searchResult) {
    let powerBases = []; //saves all bases
    searchResult.forEach((powerIndex) => {
        let currBase = []; //saves current base
        let paranthesesCount = 0;
        let previousIndex = powerIndex - 1;
        while (previousIndex >= 0) {
            if (formulas[previousIndex] == OPENPARANTHESE)
                paranthesesCount--;
            if (formulas[previousIndex] == CLOSEPARANTHESE)
                paranthesesCount++;
...
function getFactorialNumber(formulas, searchResult) {
    let numbers = [];
    let factorialSequence = 0;
    searchResult.forEach((factorialIndex) => {
        ...
        let replacement = factorial.repeat(times) + numberStr + CLOSEPARANTHESE.repeat(times);
        numbers.push({ toReplace: toReplace, replacement: replacement });
        factorialSequence = 0;
    });
    return numbers;
}
function putMultiply(formulas, val) {
...
function putMultiplyNum(formulas, val) {
    let searchResult = search(formulas, val);
    let indexes = [];
    searchResult.forEach((index) => {
        if (index != 0) {
